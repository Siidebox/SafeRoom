...
Usage:
    from saferoom.ml.features import load_and_extract
...
def _extract_from_df(df: pd.DataFrame, session_id: str,
    ...
    for _tid, track_df in df_track.groupby('tid'):
    ...
    for p in paths:
        ...
        if os.path.isdir(sp):
            ...
            if os.path.isfile(candidate):
                ...
                warnings.warn(f'Session dir without radar.csv, skipping: {sp}', stacklevel=2)
        else:
            out.append(sp)
    return out
...
def load_and_extract(csv_paths, window_size: int = WINDOW_SIZE,
    ...
    for path in csv_paths:
        ...
        if not os.path.isfile(path):
            warnings.warn(f'CSV not found, skipping: {path}', stacklevel=2)
            continue

        try:
            df = pd.read_csv(path, low_memory=False)
        except Exception as e:
            warnings.warn(f'Failed to read {path}: {e}', stacklevel=2)
            continue
...
def main():
    """Extract features from one or more session CSVs and print their shapes."""
    import argparse

    parser = argparse.ArgumentParser(
        description='Extract sliding-window features from recorded session CSVs '
                    'and report the resulting array shapes.')
    parser.add_argument('csv', nargs='+', help='Session CSV file(s), e.g. logs/ml_*.csv')
    args = parser.parse_args()

    X, X_seq, y, groups, feat_names, seq_names = load_and_extract(args.csv)
    print(f'\nX shape:     {X.shape}')
