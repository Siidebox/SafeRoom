...
from saferoom.ml.features import STRIDE, WINDOW_SIZE, load_and_extract
from saferoom.ml.train import eval_predictions, run_loso_cv, threshold_from_cv
...
def replay_rule_based(csv_paths: list) -> tuple:
    ...
    for path in csv_paths:
        ...
        if 'label' not in df.columns:
            warnings.warn(f'{path}: no "label" column, skipping rule-based replay.', stacklevel=2)
            continue
...
def plot_confusion_matrices(cm_data: dict, outdir=None):
    ...
    for ax, (name, (y_true, y_pred)) in zip(axes, cm_data.items(), strict=False):
...
def plot_feature_importance(model_path: str, feat_names: list, top_n: int = 20,
    ...
    if not os.path.isfile(model_path):
        warnings.warn(f'Model not found for feature importance: {model_path}', stacklevel=2)
        return
    model = joblib.load(model_path)
    if not hasattr(model, 'feature_importances_'):
        warnings.warn('Model has no feature_importances_ attribute.', stacklevel=2)
        return
...
def main():
    ...
    if args.compare_rule_based:
        ...
        try:
            ...
            warnings.warn(f'Rule-based replay failed: {e}', stacklevel=2)
