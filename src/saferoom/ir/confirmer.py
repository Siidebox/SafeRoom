...
class BackgroundModel:
    ...
    def __init__(self, params: IrConfirmerParams | None = None) -> None:
...
class IrConfirmer:
    ...
    def __init__(self, params: IrConfirmerParams | None = None,
                 background: BackgroundModel | None = None) -> None:
    ...
    def evaluate(self, radar_t_mono_ns: int) -> ConfirmerResult:
        ...
        last_cy = [cy for cy, ts in zip(cy_list, ts_for_centroids, strict=False)
                   if ts >= stillness_cutoff and not np.isnan(cy)]
        last_cx = [cx for cx, ts in zip(cx_list, ts_for_centroids, strict=False)
