...
- **IR coverage gap → fail-closed on one side of the room (dataset 2026-07-20/21)**: The MLX90640 does not register a warm blob for a person on one side of the room (physical left ≈ radar's wide/+X side). The radar tracks that region fine, but the IR confirmer's Rule-0 pre-window ghost veto (`ir_confirmer.py`, empty pre-window ⇒ `vetoed`) treats a real fall there as a ghost track and **vetoes** it, so the fused system misses it (downgraded to `fall_candidate`, log-only). Impact is limited to the **IR fusion metric (Fase 7.3)** — the radar-only ML detector (the primary contribution) is unaffected since it never uses IR. This is a coverage-dependent **fail-closed** limitation and is documented as the thesis' fail-closed analysis, not a bug: fusion cuts false alarms but can suppress real falls where IR has no view. Analyze fusion by the actual `x` coordinate in `radar.csv`, not by session name — some 2026-07-20 fall sessions have swapped `izq`/`dcha` names (confirmed via X-position), so the name is unreliable for the side split.
- **Single-subject dataset → session-level LOSO cannot prove subject independence**: The labeled dataset (123 sessions, ~28 fall events / ~173 fall windows, heavily imbalanced) was recorded with a **single subject** (`guillermo`). Cross-validation is Leave-One-Session-Out (LOSO) at the **session** level, not the subject level — every fold still trains and tests on the same person's radar signature, so the model may be overfitting to one individual's body/gait/fall dynamics rather than learning subject-independent fall structure. This means the reported metrics (pooled recall 0.757, 0 false alarms in replay) may be **optimistic** as an estimate of inter-person generalization. It matters because a thesis tribunal/reviewer will flag it as a generalization-validity gap. Addressed as documented **future work**: record 1–2 additional subjects to enable subject-level LOSO and validate cross-person generalization (see `docs/justificacion_modelo_ml.md`).
